...
public class TestService {
	...
	public static MedicieResponse apiTest() {
		...
		MedicieResponse result = null;
		try {
			result = RestApiUtil.ConnHttpGetType(url, data, headerData, MedicieResponse.class);
		} catch (IOException e) {
	 ...
	 private static final int THREAD_POOL_SIZE = 10;
	    ...
	    public List<MedicieResponse> medicineSearch() {
	        String url = "http://apis.data.go.kr/1471000/DrbEasyDrugInfoService/getDrbEasyDrugList";
	        String key = "52hUuwZkGcmIn8daxaXmr0bjQytOKkbKltQsPA5J5d5IcoVpvU4Kk7jwPzlIBcf9hftYnTUD9NclotFT5eo+yQ==";
	        ExecutorService executor = Executors.newFixedThreadPool(THREAD_POOL_SIZE);
	        List<CompletableFuture<MedicieResponse>> futures = new ArrayList<>();

	        for (int cnt = 1; cnt < 50; cnt++) {
	            int pageNo = cnt;
	            CompletableFuture<MedicieResponse> future = CompletableFuture.supplyAsync(() -> {
	        ...
	        List<MedicieResponse> results = new ArrayList<>();
	        for (CompletableFuture<MedicieResponse> future : futures) {
	            try {
	                MedicieResponse response = future.get();
